 ...
 */
 ...
 public class ReviewController {
        ...
        public ApiResponse<TodayReviewResponse> getTodayReviews(
                ...
                return ApiResponse
                                .success(new TodayReviewResponse(items, items.size(), 1, 0, Math.max(1, items.size())));
        }
        ...
        public ApiResponse<TodayReviewResponse> getWrongAnswers(
                        ...
                        @Parameter(hidden = true) @PageableDefault(size = 5) org.springframework.data.domain.Pageable pageable,
                        @AuthenticationPrincipal SsafyUserDetails userDetails) {

                Long userId = userDetails.getUser().getId();
                org.springframework.data.domain.Page<TodayReviewResponse.ReviewItemDto> itemPage = fsrsService
                                .getWrongAnswersWithQuestions(userId, sortType, pageable);

                return ApiResponse.success(new TodayReviewResponse(
                                itemPage.getContent(),
