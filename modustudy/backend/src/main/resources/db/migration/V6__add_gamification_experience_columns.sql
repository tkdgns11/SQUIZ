...
ADD COLUMN `current_experience` INT DEFAULT 0 AFTER `level_name`;

-- 총 출석 횟수
ALTER TABLE `user_stats`
ADD COLUMN `total_attendance` INT DEFAULT 0 AFTER `total_studies_led`;

-- 오늘 채팅으로 얻은 경험치 (일일 제한용)
ALTER TABLE `user_stats`
ADD COLUMN `today_chat_exp` INT DEFAULT 0 AFTER `total_chat_count`;

-- 마지막 채팅 날짜 (일일 제한 리셋용)
ALTER TABLE `user_stats`
ADD COLUMN `last_chat_date` DATE DEFAULT NULL AFTER `today_chat_exp`;
