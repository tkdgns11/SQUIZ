...
class ReviewControllerTest {
    ...
    @BeforeEach
    void setup() {
        User user = User.builder().email("test@example.com").nickname("tester").role(Role.USER).isActive(true).build();
    ...
    void getTodayReviews_ShouldReturnList() throws Exception {
        ...
        TodayReviewResponse.ReviewItemDto dto = new TodayReviewResponse.ReviewItemDto(
                1L, ReviewContentType.COURSE_QUESTION, 100L, 2.5, 5.0, 2, 3, 0, LocalDateTime.now(), null);
    ...
    void getWrongAnswers_ShouldReturnList() throws Exception {
        // given
        TodayReviewResponse.ReviewItemDto dto = new TodayReviewResponse.ReviewItemDto(
                2L, ReviewContentType.COURSE_QUESTION, 101L, 0.5, 8.0, 3, 5, 2, LocalDateTime.now(), null);
