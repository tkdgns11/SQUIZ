 ...
 *
 * <p>
 * 2단계 우선순위 기반 문제 선택 로직을 검증한다.
 * </p>
 *
 * <h3>테스트 대상</h3>
 * <ul>
 * <li>1단계 우선순위: 안 푼 문제가 있으면 항상 안 푼 문제를 우선 선택하는지 검증</li>
 * <li>2단계 가중치: 모든 문제를 풀었을 때 복습 예정(Due) 문제가 높은 빈도로 선택되는지 통계 검증</li>
 * <li>무한 루프: 모든 문제를 풀어도 계속 문제가 반환되는지 검증</li>
 * <li>excludeId: 방금 푼 문제가 제외되는지 검증</li>
 * </ul>
 *
 * <p>
 * <b>주의:</b> 이 테스트는 MySQL 네이티브 쿼리(RAND(), LN() 함수)를 사용하므로
 ...
 */
 ...
 class ContinuousQuizRepositoryTest {
        ...
        @Nested
        @DisplayName("2단계 우선순위 기반 선택 검증")
        class StatisticalValidation {

                @Test
                @DisplayName("안 푼 문제가 있으면 항상 안 푼 문제를 우선 선택한다 (1단계 우선)")
                void unseenQuestionsShouldAlwaysBeSelectedFirst() {
                        // given: question1은 신규(학습 기록 없음), question2/question3는 학습 완료
                        createReviewItem(testUser.getId(), question2.getId(), 100);
                        createReviewItem(testUser.getId(), question3.getId(), 50);
                        // question1: 학습 기록 없음 → 1단계(UNION ALL 첫 번째 SELECT)에서 항상 선택됨
                        entityManager.flush();
                        entityManager.clear();

                        // when: 50번 선택하여 각 문제의 선택 빈도 측정
                        ...
                        for (int i = 0; i < 50; i++) {
                                ...
                                result.ifPresent(q -> selectionCount.merge(q.getId(), 1, Integer::sum));
                        }

                        // then: question1(안 푼 문제)이 항상 선택되어야 함 (UNION ALL 1단계 우선)
                        int unseenCount = selectionCount.get(question1.getId());
                        int seenCount2 = selectionCount.get(question2.getId());
                        int seenCount3 = selectionCount.get(question3.getId());

                        assertThat(unseenCount)
                                        .as("안 푼 문제가 1단계에서 항상 선택되어야 함 (50/50)")
                                        .isEqualTo(50);
                        assertThat(seenCount2)
                                        .as("학습 완료 문제는 안 푼 문제가 있으면 선택되지 않아야 함")
                                        .isZero();
                        assertThat(seenCount3)
                                        .as("학습 완료 문제는 안 푼 문제가 있으면 선택되지 않아야 함")
                                        .isZero();
                ...
                void dueQuestionsShouldBeSelectedMoreThanCompletedQuestions() {
                        // given: 모든 문제에 학습 기록 생성 → 1단계 건너뜀 → 2단계 가중치 선택
                        UserReviewItem dueItem = createReviewItem(testUser.getId(), question1.getId(), 5);
                        dueItem.setNextReviewAt(LocalDateTime.now().minusDays(1)); // 과거 = Due → weight 5.0
                        userReviewItemRepository.save(dueItem);

                        UserReviewItem notDueItem = createReviewItem(testUser.getId(), question2.getId(), 5);
                        notDueItem.setNextReviewAt(LocalDateTime.now().plusDays(30)); // 미래 = Not Due → weight 1/6 ≈ 0.17
                        userReviewItemRepository.save(notDueItem);

                        UserReviewItem notDueItem2 = createReviewItem(testUser.getId(), question3.getId(), 50);
                        notDueItem2.setNextReviewAt(LocalDateTime.now().plusDays(30)); // 미래 = Not Due → weight 1/51 ≈ 0.02
                        userReviewItemRepository.save(notDueItem2);
                        ...
                        int notDueCount2 = selectionCount.get(question3.getId()); // Not Due: weight = 1/51 ≈ 0.02
