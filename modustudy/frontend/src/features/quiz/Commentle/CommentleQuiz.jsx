...
const CommentleQuiz = () => {
    ...
    useEffect(() => {
        const loadQuizData = async () => {
            ...
            try {
                ...
                // 로그인한 사용자는 로컬 스토리지에서 오늘의 추측 기록 불러오기
    ...
    const updateLeaderboard = async (attemptCount) => {
        // 로그인하지 않은 사용자는 게스트_XXXX
        const nickname = isLoggedIn && user?.nickname ? user.nickname : `Guest_${Date.now().toString().slice(-4)}`;
    ...
    const scoreGuide = [
        ...
        { range: '0~24', color: '#ef4444', label: '노력 필요' },
    ];
                                    ...
                                    {leaderboard.map((entry, index) => (
                                        <li key={`${entry.nickname}-${index}`} className={`leader-item rank-${index + 1}`}>
                                            ...
                                            <span className="guess-count">
                                                {entry.attempts}번째 {entry.time}초
                                            </span>
                                        </li>
                                    ))}
                                </ul>
                            ) : (
                                <div className="text-center py-4 text-gray-400 text-sm">아직 기록이 없습니다</div>
                            )}
                        </div>
                    )}
                </div>
            ...
            <Modal isOpen={showSuccess} onClose={() => setShowSuccess(false)} title="축하합니다! ??">
                <div className="flex flex-col items-center justify-center p-6 text-center">
                    ...
                    <p className="mt-2 text-gray-600">
                        총 <strong>{guesses.length}</strong>번 만에 맞추셨어요
                    </p>
                    <button onClick={() => setShowSuccess(false)} className="btn-confirm mt-6">
                        닫기
                    </button>
                </div>
            </Modal>
